...
from vfsf_spatial_only import SpatialOnlyEncoder
...
class ContrastiveJSONDataset(Dataset):
    ...
    def __getitem__(self, idx):
        entry = self.data[idx]
        paths = entry['images'][: self.max_frames]
        views = []
        for _ in range(2):
            imgs = []
            for p in paths:
...
def contrastive_loss_hard(emb_anchor, emb_pos, temperature=0.07, top_k=5):
    ...
    sim = (emb_anchor @ emb_pos.T) / temperature  # (B,B)
    sim_pos = torch.diag(sim)                     # (B,)
    losses = []
    for i in range(B):
        negs = torch.cat([sim[i, :i], sim[i, i+1:]])   # (B-1,)
...
def main():
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_loss < best_val_loss:
            ...
            }, 'best_contrastive_hard_spatial_only.pth')
